...
library(scatterpie) # pie charts on maps
```
...
#   geom_text_repel(data=subset(pc_info, sample_name %in% c("wt-msu", "KYRG-3-14", "WAC5", "HRB132", "FY5", "BEJ3", "TACH1", "TACH_CHIN14")), aes(x=PC1, y=PC2, label = sample_name), size = 4, max.overlaps = 20) +
...
```{r}
# save plot
ggsave(paste0(temp_plot, "nyc_cbp_pca1.png"), nyc_pc_plot1, height = 3, width = 6)
```
...
cbp_name_key <- left_join(data.frame(cbp_fam$V2), vcf_dat[,c("vcf_sample_name", "sample_name", "country")],
                         join_by("cbp_fam.V2" == "vcf_sample_name"))
colnames(cbp_name_key)[1] <- "vcf_sample_name" # rename column

# add region to key
cbp_name_key <- cbp_name_key %>% mutate(region = case_when(country %in% c("Switzerland","Iceland","Netherlands","Poland","Germany","Sweden","United Kingdom","France") ~ "N. Europe",
                                      country %in% c("Spain", "Greece", "Italy","Jordan", "Algeria","Turkey") ~ "MENA/Mediterranean",
                                      country %in% c("Kyrgyzstan", "Russia") ~ "C. Asia",
                                      country %in% c("China", "Taiwan") ~ "E. Asia",
                                      country == "United States" ~ "US"))
...
cap_k4 <- ancestry_cols(cbp_p, "K4") #ggplot_opt = scale_fill_manual(values = c("pop1" = "#4DBBD5FF", "pop2" = "#E64B35FF", "pop3" = "orchid1")))

# plot
cap_k4

```
This make NYC appear as its own group, while a majority of samples from Northern Europe
```{r save cbp image}
# save image
...
ea_k2 <- ancestry_cols(ea_p, "K2", ggplot_opt = scale_fill_manual(values= c("pop1" = "#B2F2FD", "pop3"= "#996330", "pop2"= "#7FC55F")))
...
#ggsave(paste0(temp_plot,"eurasian_cbp_k2.png"), ea_k2[[1]], width = 13, height = 5, units = "in")
```
...
# make just the plot show up
plot(ea_k3[[1]])
ggsave(paste0(temp_plot,"eurasian_cbp_k3_legend_bottom.png"), ea_k3[[1]], width = 13, height = 5, units = "in")
```

```{r}
# data for k = 2 on Eurasian group
ea_k5 <- ancestry_cols(ea_p, "K5")
# make just the plot show up
plot(ea_k5[[1]])
...
ea_k3_map <- eurasia + geom_point(data=ea_k3_ancestryL, aes(x=longitude, y=latitude, color = group, shape = region), size = 7, alpha = 0.8) +
  scale_color_manual(values= c("pop2" = "#99D9E4", "pop3"= "#996330", "pop1"= "#7FC55F"),
                    labels = c("pop2"= "E. Asian", pop3 = "Mediterranean", "pop1" = "N. Eurasian")) + scale_shape_manual(values = c('C. Asia'=15, 'E. Asia'=18, 'N. Europe'=16, 'MENA/Mediterranean'=17))
...
ggsave(paste0(temp_plot, "eurasian_k3_bigDots_newCols.png"),ea_k3_map, width = 13, height = 7, units = "in")
```

I would also like the map showing the 5 samples that were selected from the E. Asian population
```{r}
ea_k3_map <- eurasia + geom_point(data=subset(ea_k3_ancestryL, sample_name %in% c("DL174", "HJC419","LJ5", "LL761", "NE2")), aes(x=longitude, y=latitude, color = group, shape = region), size = 7, alpha = 0.8) +
  scale_color_manual(values= c("pop2" = "#99D9E4", "pop3"= "#996330", "pop1"= "#7FC55F"),
                    labels = c("pop2"= "E. Asian", pop3 = "Mediterranean", "pop1" = "N. Eurasian")) + scale_shape_manual(values = c('C. Asia'=15, 'E. Asia'=18, 'N. Europe'=16, 'MENA/Mediterranean'=17))

ea_k3_map

ggsave(paste0(temp_plot, "eurasian_k3_bigDots_withheld.png"),ea_k3_map, width = 13, height = 7, units = "in")
```
...
Trying to get a similar plot with scatterpie
```{r}
# set up data frame; no longer need to assign major ancestry
ea_k4_ancestryL <- left_join(ea_p[["K4"]], vcf_dat[c("vcf_sample_name","sample_name",
                                                 "longitude","latitude","latlong_approx")])

ggplot() + geom_scatterpie(data=ea_k4_ancestryL, aes(x=longitude, y = latitude), cols=paste0("V", 1:4),
                                                    alpha = 0.6)
```
...
# make just the plot show up
plot(nyc_k3[[1]])
ggsave(paste0(temp_plot,"nyc_cbp_k3.png"), nyc_k3[[1]], width = 13, height = 5, units = "in")
```
...
#nyc_k3_ancestryL$population <- factor(nyc_k3_ancestryL$population)
```

Just a map colored by population for explaining the dataset
```{r}
nyc_pops_map <- ny_map + geom_point(data=nyc_k3_ancestryL, aes(x=longitude, y=latitude, color = city), size = 3, alpha = 0.8) + scale_color_scico_d(palette = "managua") + geom_text_repel(data= subset(nyc_k3_ancestryL, city %in% c("New York", "Brooklyn", "Queens", "Flushing (Queens)")), aes(x=PC1, y=PC2, label = population), size = 2)

ggsave(paste0(temp_plot,"nyc_pops_map.png"),nyc_pops_map, width = 7, height = 3, units = "in")
```
