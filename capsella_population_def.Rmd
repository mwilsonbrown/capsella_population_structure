...
```{r}
# Roma (geographic regions)
region.cols <- c("E. Asia" = "#7E1700", "MENA/Mediterranean"= "#B58B31", "N. Europe"= "#C0E9C2", "US"= "#389CC6","C. Asia"="#023198")
...
pc_plot1 <- ggplot() +
  ...
  geom_text_repel(data=subset(pc_info, sample_name %in% c("wt-msu", "KYRG-3-14", "WAC5", "HRB132", "FY5", "BEJ3", "TACH1", "TACH_CHIN14")), aes(x=PC1, y=PC2, label = sample_name), size = 4) +
...
```{r import admixture output files}
# read in the PLINK fam file which has the sample name information in the same order as the admixture output files
ea_fam <- read.delim(paste0(admix_dir, "new_nyc/eurasian_pruned_cbp.fam"), header=FALSE)

# list the files in admixture directory matching suffix pattern
lst <- list.files(path = paste0(admix_dir,"new_nyc"), pattern = "eurasian_pruned_cbp.[0-9]+.Q") # Q files are ancestry fractions for each sample
...
```{r}
# data for k = 2 on Eurasian group
ea_k3 <- ancestry_cols(ea_p, "K3", ggplot_opt = scale_fill_manual(values= c("pop2" = "#8C0172", "pop3"= "#996330", "pop1"= "#7FC55F", "NYC"= "#B2F2FD")))
# make just the plot show up
plot(ea_k3[[1]])
...
```{r}
# data for k = 2 on Eurasian group
ea_k4 <- ancestry_cols(ea_p, "K4", ggplot_opt = )
# make just the plot show up
plot(ea_k4)
...
ea_k3_map <- eurasia + geom_point(data=ea_k3_ancestryL, aes(x=longitude, y=latitude, fill = group, color = latlong_approx), shape=21, size = 7) +
  scale_fill_manual(values= c("pop2" = "#8C0172", "pop3"= "#996330", "pop1"= "#7FC55F", "NYC"= "#B2F2FD"),
                    labels = c("pop2"= "E. Asian", pop3 = "Mediterranean", "pop1" = "N. Eurasian"))
...
```{r import admixture output files}
# read in the PLINK fam file which has the sample name information in the same order as the admixture output files
nyc_fam <- read.delim(paste0(admix_dir, "new_nyc/nyc_pruned_cbp.fam"), header=FALSE)

# list the files in admixture directory matching suffix pattern
lst <- list.files(path = paste0(admix_dir, "new_nyc"), pattern = "nyc_pruned_cbp.[0-9]+.Q") # Q files are ancestry fractions for each sample
...
```{r}
# data for k = 2 on Eurasian group
nyc_k3 <- ancestry_cols(nyc_p, "K3",ggplot_opt = scale_fill_manual(values= c("pop2" = "#8C0172", "pop3"= "#996330", "pop1"= "#7FC55F", "NYC"= "#B2F2FD")))
# make just the plot show up
plot(nyc_k3[[1]])
#ggsave(paste0(temp_plot,"nyc_cbp_k3.png"), nyc_k3, width = 13, height = 7, units = "in")
```
Something is up here with the colors. Should be mostly green.
