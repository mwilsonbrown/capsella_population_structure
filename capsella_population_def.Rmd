...
```
# Capsella bursa-pastoris Principle Components
```{r}
...
# rough plot
pc_plot <- ggplot() + 
  geom_point(data=pc_info, aes(x=PC1, y=PC2,color = citation)) + theme(legend.position = "bottom") #+ geom_text(label = sample_name, color = "black", nudge_y = .5)
```

```{r}
pc_plot
```
The above PCA incudes Capsella bursa-pastoris from an F2 mapping population that is probably influencing a lot of the variation along PC1 and PC2 here.
...
cap_k3 <- ancestry_bars(cap_ancestry_propsK, K = K, sample_names_fam=cap_info_fam)
# plot
cap_k3
...
cap_k3_anc_info <- ancestry_location2(cap_k3$data, full_info)
