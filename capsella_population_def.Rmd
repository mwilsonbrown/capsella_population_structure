...
cap_fam <- read.delim(paste0(admix_dir, "all_pruned_cbp.fam"), header=FALSE)
...
ggsave(paste0(temp_plot, "eurasian_k2_bigDots.png"),ea_k3_map, width = 13, height = 7, units = "in")
```
...
```{r}
ea_k4_map <- eurasia + geom_point(data=k4_ancestryL, aes(x=longitude, y=latitude, fill = group, color = latlong_approx), shape=21, size = 7) 
...
```{r cv_error K groups}
# read in Cross Validation (cv) error log
nyc_cv <- read.delim(paste0(admix_dir, "eua_cv_error.log"), header = FALSE, sep = ":")
# extract K groups from string
nyc_cv$K_groups <- str_extract(nyc_cv$V1, "[0-9]+")
...
We are interested in how the Admixture group containing genotypes from northern Europe compare to NYC. I run another admixture analysis only including individuals that have 60% or more ancestry proportion belonging to Population 3 when K=3 (Red individuals above).
