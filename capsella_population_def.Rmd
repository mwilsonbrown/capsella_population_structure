...
```{r}
ggplot()+ geom_sf(data= states, fill="gray93")+ 
  coord_sf(xlim = c(-74.5, -73),  ylim = c(40.5, 41), expand = FALSE) + 
  geom_point(data = pc_info, aes(x=longitude, y= latitude), fill = PC2)
  theme_light() + 
...
```{r}
# assign sample to majority ancestry
ea_k5_ancestry <- assign_ancestry(ea_p, "K5")

# join with location data
ea_k5_ancestryL <- left_join(ea_k5_ancestry, vcf_dat[c("vcf_sample_name","sample_name",
                                                 "longitude","latitude","latlong_approx")])
```

```{r}
ea_k5_map <- eurasia + geom_point(data=ea_k5_ancestryL, aes(x=longitude, y=latitude, color = group, shape = region), size = 7, alpha = 0.8) +
...
```{r}
ggsave(paste0(temp_plot, "eurasian_k5_map.png"),ea_k4_map, width = 13, height = 7, units = "in")
```
...
```{r pca eigenstuff}
# read in eigenvec and eigenval data
vec <- read.csv(paste0("~/Documents/PhD/Research/capsella_population_structure/admix_out_data/", "nyc_neu/nyc_neu_pruned.eigenvec"), sep = "\t", header = T) #values on each PC

val <-  read.csv(paste0("~/Documents/PhD/Research/capsella_population_structure/admix_out_data/", "nyc_neu/nyc_neu_pruned.eigenval"), sep = "\t", header = F) #PC variances explained
