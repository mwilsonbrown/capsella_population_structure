...
```{r}
ny_map + geom_point(data=pc_info, aes(x=longitude, y=latitude, color = PC3), size = 3, alpha = 0.8)
```
...
nyc_neu_pc_plot2 <- ggplot() +
  ...
  scale_color_discrete(labels = c("United States" = "NYC Metro")) +
  geom_text_repel(data=subset(pc_info, country %in% c("China")), aes(x=PC1, y=PC3, label = sample_name), max.overlaps = 50, size = 2) +
  theme_classic() +
  labs(x=paste0("PC1",percentage[1]), y= paste0("PC3",percentage[3]))
