...
One of the first things I would like to do is back fill locations for data points in the VCF. For example, many from the Douglass paper do not have lat and long, but they have general cities in the notes. I would also like to fill in country names for locations that do have a latitude and longitude.
...
pc_plot1 <- ggplot(data=pc_info, aes(x=PC1, y=PC2, label = sample_name)) + 
  geom_point(aes(color = as.factor(population))) + geom_text(hjust = 1, vjust = 0.2, size = 2)
```
...
pc_plot5 <- ggplot(data=pc_info, aes(x=longitude, y=latitude, label = sample_name)) + 
  geom_point(aes(color = PC2)) + geom_text(hjust = 1, vjust = 0.2, size = 2)

pc_plot5
```

It seems like PC1 largely captures the East Asian population separating out from everyone else, and PC2 is picking up on the Mediterranean populations.

For NYC, PC1 is capturing variation driven by population 24, while pc2 is capturing variation driven by population 14.
...
```{r import admixture output files}
# read in the PLINK fam file which has the sample name information in the same order as the admixture output files
eu_fam <- read.delim(paste0(admix_dir, "eurasian_pruned_cbp.fam"), header=FALSE)

# read in Cross Validation (cv) error log
eu_cv <- read.delim(paste0(admix_dir, "cv_error.log"), header = FALSE, sep = ":")
