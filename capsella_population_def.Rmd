...
ea_k5_map <- eurasia + geom_point(data=ea_k5_ancestryL, aes(x=longitude, y=latitude, color = group, shape = region), size = 7, alpha = 0.8) +
  ...
  geom_text_repel(data=subset(ea_k5_ancestryL, sample_name %in% c("RK32", "KYRG-3-14","QD325","wt-msu", "BEJ3", "FY5", "TACH1", "TACH-CHIN14", "HRB138", "HRB135", "HRB132", "QD325", "HD63")), aes(x=longitude, y=latitude, label = sample_name), max.overlaps = 50, size = 3)
...
# read in eigenvec and eigenval data
vec <- read.csv(paste0(admix_dir, "nyc_neu_pruned.eigenvec"), sep = "\t", header = T) #values on each PC

val <-  read.csv(paste0(admix_dir, "nyc_neu_pruned.eigenval"), sep = "\t", header = F) #PC variances explained
